...
with st.sidebar:
    ...
    if 'portfolio' not in st.session_state:
        st.session_state.portfolio = [
            ...
            {'symbol': 'GC=F', 'type': 'Emtia', 'name': 'Altin'},
            {'symbol': 'SI=F', 'type': 'Emtia', 'name': 'Gumus'},
            {'symbol': 'GLD', 'type': 'Emtia', 'name': 'Altin ETF'},
            {'symbol': 'BTC-USD', 'type': 'Kripto', 'name': 'Bitcoin'},
            {'symbol': 'ETH-USD', 'type': 'Kripto', 'name': 'Ethereum'},
            {'symbol': 'SOL-USD', 'type': 'Kripto', 'name': 'Solana'},
        ]

    st.markdown("### ➕ Varlik Ekle")
    new_symbol = st.text_input("Sembol", placeholder="orn: MSFT", key="new_sym").upper().strip()
    new_name = st.text_input("Isim (opsiyonel)", placeholder="orn: Microsoft", key="new_name")
    new_type = st.selectbox("Tur", ['BIST', 'US', 'Emtia', 'Kripto', 'Fon'], key="new_type")
...
for asset in st.session_state.portfolio:
    ...
    with st.container():
        ...
        if df is not None and not df.empty:
            ...
            fig.update_layout(title=symbol + " - 3 Aylik Grafik", xaxis_title="Tarih", yaxis_title="Fiyat", height=400, template="plotly_dark", showlegend=True, xaxis_rangeslider_visible=False)
