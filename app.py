...
st.markdown("<style>.main{padding:0rem 1rem}.stButton>button{width:100%;border-radius:10px;height:3rem;font-size:1.1rem}.metric-card{background:#1e1e1e;padding:1rem;border-radius:10px;margin:0.5rem 0}.buy-signal{color:#00ff88;font-weight:bold;font-size:1.2rem}.sell-signal{color:#ff4444;font-weight:bold;font-size:1.2rem}.hold-signal{color:#ffaa00;font-weight:bold;font-size:1.2rem}.price-big{font-size:2rem;font-weight:bold;color:#fff}.change-positive{color:#00ff88;font-size:1.2rem}.change-negative{color:#ff4444;font-size:1.2rem}.cat-header{color:#4a9eff;font-size:1.1rem;font-weight:bold;margin-top:1rem}</style>", unsafe_allow_html=True)
...
def fetch_price(symbol):
    try:
        ...
        if not data.empty:
            ...
            return {'price': round(last['Close'], 4), 'change': round(change, 2), 'volume': int(last['Volume']), 'high': round(data['High'].max(), 4), 'low': round(data['Low'].min(), 4), 'open': round(data['Open'].iloc[0], 4)}
    except:
        pass
    return None
    ...
    if st.session_state.selected_categories:
        ...
        for category in st.session_state.selected_categories:
            if category in ALL_ASSETS:
                st.markdown("<div class='cat-header'>" + category + "</div>", unsafe_allow_html=True)
...
if not st.session_state.portfolio:
    ...
    st.markdown("**Nasil kullanilir:** 1. Sol menuden **Kategori** secin (BIST, Nasdaq, vb.)  2. Acilan listeden **hisse senetlerini** secin  3. Ana ekranda **anlik fiyat** ve **sinyaller** gorun  4. **Grafikler** ile teknik analiz yapin")
else:
        ...
        with st.container():
            ...
            if df is not None and not df.empty:
                ...
                if 'SMA_20' in df.columns:
                    fig.add_trace(go.Scatter(x=df.index, y=df['SMA_20'], name='SMA20', line=dict(color='orange', width=1)), row=1, col=1)
                if 'SMA_50' in df.columns:
                    fig.add_trace(go.Scatter(x=df.index, y=df['SMA_50'], name='SMA50', line=dict(color='blue', width=1)), row=1, col=1)
                fig.add_trace(go.Bar(x=df.index, y=df['Volume'], name='Hacim', marker_color='gray', opacity=0.3), row=2, col=1)
                fig.update_layout(title=symbol + " - 3 Aylik", xaxis_title="Tarih", yaxis_title="Fiyat", height=400, template="plotly_dark", showlegend=True, xaxis_rangeslider_visible=False)
...
st.markdown("*⚠️ Bu uygulama egitim amaclidir. Yatirim tavsiyesi degildir.*")
